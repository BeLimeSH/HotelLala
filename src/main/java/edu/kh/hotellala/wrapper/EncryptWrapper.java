package edu.kh.hotellala.wrapper;
...
public class EncryptWrapper extends HttpServletRequestWrapper {
	...
	// - 클라이언트 요청 객체 HttpServletRequestWrapper를 오버라이딩 하는 방법을 제공하는 클래스 
	
	
	//생성자가 작성되어 있으면 
	//컴파일러가 기본 생성자를 자동으로 추가 안함 
	
	// -> Wrapper 클래스 생성 시 반드시 HttpServletRequest 객체를 매개 변수로 전달해야한다.
	...
	private String getSha512(String pw) {
		...
		try {
			// 2.SHA-512 방식의 해시 함수를 수행 할 수 있는 객체를 얻어옴 
			md =MessageDigest.getInstance("SHA-512");
			
			// 3. md를 이용해 암호화 진행 할 수 있도록 pw(기존 비번) byte[] 형태로 변환 
		...
		return encryptPw;// 암호와 안된걸 받아서 암호화 되게 해서 내보내겠다.
